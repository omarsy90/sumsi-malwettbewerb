console.log('hi ich bin in bilder.js');
...
async function getInfoSubmission() {
  ...
  const response = await fetch(`http://localhost:3000/submission/${subId}`);
...
async function vote(id,data) {
  ...
  const response = await fetch(`http://localhost:3000/submission/${id}/voting`, {
    method: 'POST',
